...
class MAX7219_Dot_Matrix
  ...
  // RSB: Renamed: Eclipse didn't allow pointer to class with non leading capital with trailing underscore
  bitBangedSPI *BbSPI_;
  ...
  public:
    ...
    void sendSmooth (const unsigned char * s, const int pixel);
    void setIntensity (const byte amount);  // 0 to 15

    // RSB New functions to allow LEDs to switch off completely
    void shutdown(bool);
